    ...
    .then(v => {
        if(v.status != 'successful') {
            ...
            v.data.map(item => {
                ...
                $html += "<td><button type='button' class='btn btn-primary btn-sm' onclick='show_leave("+item.id+", "+item.cname+", "+item.agent_cname+", "+item.leave_name+")'>查</button></td>";
                $html += "</tr>";
...
const show_leave = (apply_id, cname, agent_cname, leave_name) => {
    alert(apply_id);
